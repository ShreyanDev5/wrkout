  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        style={Object.fromEntries(Object.entries(config).flatMap(([key, value]) => [[`--color-${key}`, value.color]]))}
      >
        {children}
      </div>
            ...
            <div key={entry.dataKey} className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-1">
                <div
                  ...
                  style={{
                    backgroundColor: config[entry.dataKey]?.color,
                  }}
                />
                <span className="text-xs text-muted-foreground">{config[entry.dataKey]?.label}</span>
              </div>
